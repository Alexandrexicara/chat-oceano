...
import { getMessages, sendMessage as sendApiMessage, uploadFile } from '../services/api'
...
export function Chat() {
  const { user, logout, refreshContacts, contacts: ctxContacts } = useAuth()
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Contatos já são carregados pelo AuthContext — apenas formata para exibição local
        setLocalContacts(
          (ctxContacts || []).map(contact => ({
            id: contact.id,
            name: contact.name,
  ...
  useEffect(() => {
    // Socket desabilitado — sem backend separado no Next.js
    // Mensagens novas chegam via polling ao reabrir o chat
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.colors.background }}>
      ...
      {showWhatsAppSync && (
        <WhatsAppSync onClose={async () => { setShowWhatsAppSync(false); await refreshContacts(user?.id) }} />
      )}
