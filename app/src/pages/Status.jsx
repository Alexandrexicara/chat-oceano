...
import { getStatusMessages, sendMessage as sendApiMessage, uploadFile } from '../services/api'
...
export function Status() {
  ...
  useEffect(() => {
    const loadStatuses = async () => {
      if (!user?.id) return
      try {
        // Buscar status do usuário + contatos (não todo o oceano)
        const oceanoMessages = await getStatusMessages(user.id)
  ...
  useEffect(() => {
    // Socket.io só conecta se houver backend separado (produção com Render)
    // No Next.js standalone não há socket server — o polling via HTTP é suficiente
    ...
    try {
      ...
      if (statusForm.mediaUrl && statusForm.mediaUrl.startsWith('blob:')) {
        ...
        try {
          ...
          // Aceita url, path ou filename
          mediaUrlFinal = uploadResult.url || uploadResult.path || (uploadResult.filename ? `/uploads/${uploadResult.filename}` : null) || mediaUrlFinal
